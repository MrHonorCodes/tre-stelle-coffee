...
interface ProductForCart extends SanityDocument {
	// Or use your existing SanityProduct interface if imported
	_id: string;
	name: string;
	price: number;
...
type CartContextType = {
	...
	addToCart: (
		...
		quantity: number,
		options?: { [key: string]: string }
	) => { success: boolean; quantityAdded: number; finalQuantity: number; message?: string };
...
export const CartProvider = ({ children }: { children: ReactNode }) => {
	...
	const addToCart = (
	...
	): { success: boolean; message: string; quantityAdded: number; finalQuantity: number } => {
		let resultState = {
			success: false,
		...
		setCartItems((prevItems) => {
			...
			if (existingItemIndex !== -1) {
				...
				if (quantityChange >= 0) {
					// If quantity can be increased or stays the same (at max)
					...
					resultState = {
						success: true,
						message:
							quantityChange < quantityToAdd && quantityToAdd > 0
				...
				if (currentTotalQuantityInCart + quantityToAdd <= MAX_TOTAL_ITEMS_IN_CART) {
					...
					resultState = {
						success: true,
						message:
							`${quantityForNewItem} ${product.name}(s) added to cart.` +
							(quantityForNewItem < quantityToAdd
								? ` Max ${MAX_QUANTITY_PER_ORDER} per item.`
								: ''),
	...
	const removeFromCart = (productId: string, options?: { [key: string]: string }) => {
		setCartItems((prevItems) => {
			...
			return prevItems.filter(
				(item) => generateCartItemKey(item.productId, item.options) !== itemKey
			);
		});
	};
	...
	const updateQuantity = (
		...
		console.log('[CartContext updateQuantity] Called with:', {
			productId,
		...
		setCartItems((prevItems) => {
			const itemKey = generateCartItemKey(productId, options);
			console.log(
				'[CartContext updateQuantity] prevItems:',
				prevItems,
				'targetKey:',
				itemKey,
				'newQuantity:',
			...
			const updatedItems = prevItems.map((item) => {
				if (generateCartItemKey(item.productId, item.options) === itemKey) {
					console.log(
						'[CartContext updateQuantity] Updating item:',
						item,
						'to quantity:',
						quantity
					);
