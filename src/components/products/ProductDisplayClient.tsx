	...
	>
		{halfFilled ? (
			<>
				...
				/>
				<path
					d="M10 2.236l-1.07 3.292a1 1 0 00-.95.69H4.518c-.97 0-1.371 1.24-.588 1.81l2.8 2.034a1 1 0 00.364 1.118l-1.07 3.292c-.3.921.755 1.688 1.54 1.118l2.8-2.034a1 1 0 001.175 0l2.8 2.034c.784.57 1.838-.197 1.539-1.118l-1.07-3.292a1 1 0 00-.364-1.118l2.8-2.034c.783-.57.38-1.81-.588-1.81H12.47a1 1 0 00-.95-.69L10 2.236z"
					opacity="0.5"
				/>
			</>
		) : (
...
export default function ProductDisplayClient({ product }: { product: SanityProduct }) {
	...
	const handleAddToCart = () => {
		...
		setToastMessage(
			result.message ||
				(result.success ? `${product.name} updated in cart.` : 'Could not add item to cart.')
		);

		if (
			result.quantityAdded === quantity &&
			result.finalQuantity <= MAX_QUANTITY_PER_ORDER &&
			quantity > 0
		) {
			setToastType('success');
		} else if (result.quantityAdded >= 0 && result.quantityAdded < quantity) {
			// Used >= 0 for quantityAdded in case none were added but it was a clamp.
	...
	const handleSubmitReview = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			setReviewFormMessage({
				type: 'success',
				text: 'Review submitted successfully! It will appear once approved.',
			});
	...
	return (
		<div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 items-start">
			...
			{showConfirmation && (
				<div
					className={`fixed bottom-10 left-1/2 -translate-x-1/2 ${toastBgColor} px-4 py-3 rounded shadow-lg z-50 transition-opacity duration-300 ease-in-out`}
				>
					<strong className="font-bold">
						{toastType === 'success' ? 'Success!' : toastType === 'warning' ? 'Note:' : 'Error!'}
					</strong>
			...
			<div className="flex flex-col">
				<span className="text-sm text-gray-500 mb-1 uppercase tracking-wider">
				...
				{product.reviews && product.reviews.length > 0 && (
					<div className="flex items-center mb-4">
						...
						<span className="ml-2 text-sm text-gray-600">
							({averageRating.toFixed(1)} based on {product.reviews.length} review
					...
					<div className="mb-6">
						<div className="flex items-center">
							...
							<div className="flex items-center border border-gray-300 rounded-md">
								...
								<span className="px-4 py-1 border-l border-r border-gray-300 text-center w-12 select-none">
									{quantity}
								</span>
								<button
									onClick={() => handleQuantityChange(1)}
						...
						<p className="text-xs text-gray-500 mt-1 ml-1">
							Max {MAX_QUANTITY_PER_ORDER} per order
						</p>
					</div>
				)}
						...
						{product.reviews.map((review) => (
							<div key={review._id} className="bg-white p-6 rounded-lg shadow">
								<div className="flex items-center mb-2">
									...
									{review.title && (
										<h3 className="ml-3 text-lg font-semibold text-gray-800">{review.title}</h3>
									)}
								</div>
				...
				<div className="mt-8">
					...
					{showReviewForm && (
						<form onSubmit={handleSubmitReview} className="mt-6 bg-white p-6 rounded-lg shadow">
							...
							{reviewFormMessage && (
								<div
									className={`p-3 mb-4 rounded text-sm ${reviewFormMessage.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
								>
							...
							<div className="mb-4">
								<label
									htmlFor="reviewAuthorEmail"
									className="block text-sm font-medium text-gray-700 mb-1"
								>
									Your Email <span className="text-xs text-gray-500">(Not published)</span>
								</label>
								<input
									type="email"
									id="reviewAuthorEmail"
